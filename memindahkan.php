        ...
        <div class="card">
            <div class="form-group">
                ...
                <select name="arsip_id" class="form-control" required>
                    ...
                    $query_arsip = mysqli_query($koneksi, "SELECT id, kode_arsip, perihal, ruangan, no_rak FROM arsip");
                    while($row = mysqli_fetch_assoc($query_arsip)) {
                        echo "<option value='".$row['id']."'>".$row['kode_arsip']." - ".$row['perihal']." (Posisi: ".$row['ruangan']."/".$row['no_rak'].")</option>";
                    }
                    ?>
                </select>
            </div>
